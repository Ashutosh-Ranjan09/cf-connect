import { Problem, Submission, Contest, Friend, LeaderboardEntry } from '@/app/providers';
...
const problemTags = [
  ...
  'meet-in-the-middle', 'schedules', '2-sat', 'chinese remainder theorem'
];

const languages = [
  'GNU C++17', 'GNU C++14', 'GNU C++11', 'GNU C++', 'GNU C++20', 
  'MS C++', 'Mono C#', 'Python 3', 'Python 2', 'Java 8',
  'Java 11', 'Java 17', 'Kotlin', 'Rust', 'Go', 'Ruby', 'PHP'
];
...
export const mockSubmissions: Submission[] = Array.from({ length: 150 }, (_, i) => {
  ...
  return {
    ...
    verdict: isAccepted ? 'OK' : verdicts[Math.floor(Math.random() * (verdicts.length - 1)) + 1],
...
export const mockContests: Contest[] = Array.from({ length: 20 }, (_, i) => {
  ...
  const types: Contest['type'][] = ['Div. 1', 'Div. 2', 'Div. 3', 'Div. 4', 'Educational', 'Global'];
  ...
  return {
    ...
    ratingDelta: !isUpcoming ? Math.floor(Math.random() * 200) - 100 : undefined
  };
});

// Generate mock friends
export const mockFriends: Friend[] = Array.from({ length: 15 }, (_, i) => {
  const ratings = [800, 1200, 1400, 1600, 1800, 2000, 2200, 2400, 2600, 2800, 3000, 3200, 3400];
  ...
  const activityType = activityTypes[Math.floor(Math.random() * activityTypes.length)] as 'SOLVED' | 'PARTICIPATED' | 'RANKED_UP';
  ...
  if (activityType === 'SOLVED') {
    ...
    const contest = mockContests[Math.floor(Math.random() * mockContests.length)];
    recentActivity.contestId = contest.id;
    recentActivity.contestName = contest.name;
  }
  
  const handles = [
    'tourist', 'Petr', 'Um_nik', 'scott_wu', 'ecnerwala', 'Benq', 'ksun48', 
    'Radewoosh', 'Errichto', 'jiangly', 'tfg', 'maroonrk', 'PurpleCrayon', 
    'ko_osaga', 'Geothermal', 'Egor', 'neal', 'dacin21', 'aid', 'Swistakk', 
    'Petr', 'tourist', 'ksun48', 'Benq', 'ecnerwala', 'Um_nik', 'Radewoosh'
  ];
...
export const mockLeaderboard: LeaderboardEntry[] = Array.from({ length: 50 }, (_, i) => {
  ...
  return {
    ...
    avatar: friend ? friend.avatar : `https://avatars.githubusercontent.com/u/${20000 + i}?v=4`,
    isFriend
  };
});
...
export const getRatingOverTime = (): { date: string; rating: number }[] => {
  const contestsWithRating = [...mockContests]
    .filter(contest => contest.phase === 'FINISHED' && contest.ratingDelta !== undefined)
...
export const getDifficultyBreakdown = (): { rating: string; solved: number }[] => {
  const ratingRanges = [
    '800-999', '1000-1199', '1200-1399', '1400-1599', '1600-1799', 
    '1800-1999', '2000-2199', '2200-2399', '2400-2599', '2600-2799',
    '2800-2999', '3000-3199', '3200+'
  ];
